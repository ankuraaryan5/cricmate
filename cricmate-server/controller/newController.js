...
export const updateScore = async (req, res) => {
  try {
    ...
    // Validate the input
    if (
      inning < 1 || over < 0 || ball < 1 || ball > 6 ||                 
      typeof batter1 !== "string" || !batter1.trim() ||  
      typeof batter2 !== "string" || !batter2.trim() ||  
      typeof bowler !== "string" || !bowler.trim()
    ) {
      return res.status(400).json({ message: "Invalid values provided for score update" });
    }
...
export const updateMatchDetails = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update match details", error: error.message || error });
    }
